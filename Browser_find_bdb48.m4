dnl Copyright (c) 2013-2015 The Browser Coin Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

AC_DEFUN([BROWSER COIN_FIND_BDB48],[
  ...
  if test "$use_bdb" = "no"; then
    ...
    for searchpath in $bdbdirlist ''; do
      ...
      AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        ...
        #if !(BDB_VERSION_MAJOR == 4 && BDB_VERSION_MINOR == 8)
          #error "failed to find bdb 4.8"
        #endif
      ]])],[
    ...
    if test "$bdbpath" = "X"; then
      ...
      BROWSER COIN_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bwpath}],bd_cxx)
      AC_ARG_WITH([incompatible-bw],[AS_HELP_STRING([--with-incompatible-bw], [allow using a bd version other than 4.8])],[
      ...
      BROWSER COIN_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdb48path}],db_cxx)
      bdbpath="${bdb48path}"
      use_bdb=yes
    fi
  else
